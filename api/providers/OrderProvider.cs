...
public class OrderProvider : BaseProvider<Order>
{
    ...
    public Order UpdateOrderItems(Guid orderId, List<OrderItemRequest> updatedItems)
    {
        Order existingOrder = _db.Orders.Include(o => o.OrderItems).FirstOrDefault(o => o.Id == orderId);
        
        if (existingOrder == null) throw new ApiFlowException($"Order not found for id '{orderId}'");

        _db.OrderItems.RemoveRange(existingOrder.OrderItems);

        existingOrder.OrderItems = updatedItems.Select(oi => new OrderItem
        {
            ItemId = oi.ItemId,
            Amount = oi.Amount
        }).ToList();
